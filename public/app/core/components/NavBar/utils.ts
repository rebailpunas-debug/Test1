...
export const enrichConfigItems = (
  ...
  // Add File Upload to configuration items for Editor and Admin roles
  const isEditorOrAdmin = user?.orgRole === 'Editor' || user?.orgRole === 'Admin';
  if (isEditorOrAdmin && !items.some((item) => item.id === 'fileupload')) {
    items.push({
      text: 'File Upload',
      icon: 'cloud-upload',
      id: 'fileupload',
      url: getConfig().appSubUrl + '/file-upload',
